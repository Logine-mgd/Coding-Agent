...
namespace AIAgentMvc.Controllers
{
    public class ChatController : Controller
    {
        ...
        public async Task<JsonResult> Send([FromBody] ChatSendRequest req)
        {
            ...
            // Post-process responses to extract code snippets and create concise preview
            var processed = responses.Select(r =>
            {
                var full = r ?? string.Empty;
                var snippet = ExtractSnippet(full, 20, 800, out bool hasMore);
        ...
        // Extracts a readable snippet from a model response. If the response contains
        // a fenced code block (```), returns the first code block truncated to maxLines.
        // Otherwise returns the first maxChars characters. Sets hasMore when truncated.
        private static string ExtractSnippet(string text, int maxLines, int maxChars, out bool hasMore)
        {
            hasMore = false;
            if (string.IsNullOrEmpty(text)) return string.Empty;

            // Prefer fenced code block — return full code block to avoid trimming code
            var fenceMatch = Regex.Match(text, "```(?:[a-zA-Z0-9+-]*)\\r?\\n([\\s\\S]*?)```", RegexOptions.Multiline);
            if (fenceMatch.Success)
            {
                var code = fenceMatch.Groups[1].Value.Replace("\r\n", "\n").TrimEnd();
                hasMore = false; // return full code for readability
                return code;
            }

            // No fenced block — trim by characters but preserve newlines up to maxLines
            var normalized = text.Replace("\r\n", "\n");
            var nlLines = normalized.Split('\n');
            if (nlLines.Length > maxLines)
            {
                hasMore = true;
                return string.Join('\n', nlLines.Take(maxLines)) + "\n...";
            }
